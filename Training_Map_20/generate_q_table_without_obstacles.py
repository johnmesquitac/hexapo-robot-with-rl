...
alpha = 0.009  # learning rate, learn more quickly if alpha is closer to 1
gamma = 0.33  # use a higher gamma for smaller spaces because we value later rewards rather than former rewards
epsilon = 0.08  # agent can do 100% exploitation (0) or 100% exploration (1)
...
def evaluate_training(state):
    ...
    for a_idx in action_space:
        ...
        sns.heatmap(Q[:,a_idx].reshape((25,1)), cmap='hot', vmin=np.min(Q[:,:]), vmax=np.max(Q[:,:]))
        if a_idx == 0:
            direction = 'Up'
        elif a_idx ==1:
            direction = 'Left'
        ...
        plt.ylabel('States')
    plt.tight_layout()
    plt.savefig('imgs/without_obstacles/heatmap/heatmap'+state+'.png')
    ...
    plt.tight_layout()
    plt.savefig('imgs/without_obstacles/heatmap/heatmapQ'+state+'.png')
    ...
    if state == '24':
        ...
        plt.savefig("imgs/without_obstacles/boxplot/boxplot.png")
