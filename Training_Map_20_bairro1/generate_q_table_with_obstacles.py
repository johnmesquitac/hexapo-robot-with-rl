...
def evaluate_training(state, steps, rewards, penalties, training_end, training_start):
    ...
    x = [[x for x in range(NUM_EPISODES)]]*2
    print(x)
    y= rewards
    z = steps
    fig = go.Figure(data=[go.Surface(z=x, x=z, y=y)])
    fig.show()
    ...
    '''    fig = plt.figure()
        ...
        for a_idx in action_space:
            ...
            sns.heatmap(Q[:,a_idx].reshape((400,1)), cmap='hot', vmin=np.min(Q[:,:]), vmax=np.max(Q[:,:]))
            if a_idx == 0:
                direction = 'Up'
            elif a_idx ==1:
                direction = 'Left'
...
def plot_matrix(matrix, x_size, y_size, state):
    ...
    plt.tight_layout()
    plt.savefig("imgs/with_obstacles/Q/Q"+state+".png")
