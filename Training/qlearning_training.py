# pylint: disable=locally-disabled, multiple-statements, fixme, line-too-long, unused-variable
import itertools
import matplotlib
import numpy as np
import pandas as pd
...
epsilon = 0  # agent can do 100% exploitation (0) or 100% exploration (1)
...
def training_agent(q_table, enviroment, num_episodes, env_size):
    ...
    for _ in range(num_episodes):
        ...
        while int(reward) < 20:
            state, reward, old_state = update(
                q_table, enviroment, state, old_state, num_episodes)
            total_reward += reward
            if state != old_state:
                epochs += 1
            if reward == -10:
...
def evaluate_training():
    ...
    plt.savefig('imgs/trainingprocess.png')
...
def main():
    ...
    evaluate_training()
    print(Q)
    with open('pickle/q_table.pickle', "wb") as write:
        pickle.dump(Q, write)
